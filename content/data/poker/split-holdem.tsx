...
export const pokerSplitHoldemGuide: PokerPageContent = {
  ...
  introduction: {
    ...
    paragraphs: [
      ...
      "テキサス・ホールデムの経験者であれば、スプリット・ホールデムのルールはすぐに理解できるでしょう。主な違いは、その名の通り「スプリット（分割）」されるボードにあります。テキサス・ホールデムの経験がない方でも、[テキサス・ホールデムのルールを確認](https://www.casinotsu.com/poker)すれば、スプリット・ホールデムへの理解も深まります。</>,
    ],
  },
    ...
    subsections: <a href="https://casinotsu.com/reviews/pokerstars \"ポーカースターズのレビューへ\"" target="_blank" rel="noopener noreferrer">
      {
        ...
        paragraphs: [
          "スプリット・ホールデムをプレイするなら、CasinoTsu が最もおすすめなのは [PokerStars / ポーカースターズ](https://www.casinotsu.com/poker) です！ PokerStarsでは、プレイしたいゲームの種類とステーク（賭け金レベル）を選択するだけで、ゲームソフトウェアが自動的に空いている席を見つけてくれます。もしすぐにプレイできる席がない場合は、待機リストに登録され、席が空き次第参加できます。</>,
  ...
  summary: {
    ...
    paragraphs: [
      ...
      "PokerStarsでこのユニークなゲームを体験し、あなたのポーカー戦略に新たな次元を加えてみてはいかがでしょうか。ただし、その特徴的なゲームプレイゆえの、より高い変動性には注意が必要です。CasinoTsu は、常に正確な情報と[責任あるプレイ](https://www.casinotsu.com/poker)を推奨いたします。</>,
    ],
  },

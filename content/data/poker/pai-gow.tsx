...
export const paiGowPokerGuide: PokerPageContent = {
  ...
  introduction: {
    ...
    paragraphs: [
      "パイ・ゴウは、シンプルながらも奥深い戦略性を持つカードゲームです。その起源は中国のドミノゲーム「**牌九（パイ・ゴウ）**」にあり、アメリカでポーカーの要素が加わり、「パイ・ゴウ・ポーカー」として親しまれるようになりました。現在では、多くの評判の高い**[オンラインカジノ](LINK_NEEDS_REPAIR)** でこのエキサイティングなゲームをプレイすることが可能です。</>,
  ...
  rulesSection: {
    ...
    paragraphs: [
      "パイ・ゴウは、標準的な52枚のトランプにジョーカー1枚を加えた、**合計53枚のカードを使用してプレイ**されます。ゲームの基本となるのは、**[クラシックポーカー](LINK_NEEDS_REPAIR)** の役の組み合わせです。通常、ゲームは6人のプレイヤーと1人のディーラーで進行し、プレイヤーがベットを行うことから始まります。</>,
  ...
  playNowSection: {
    ...
    paragraphs: [
      "CasinoTsuが**推薦するオンラインカジノ**の中には、パイ・ゴウ・ポーカーを提供しているところが複数ございます。もちろん、パイ・ゴウだけでなく、他のクラシックカードゲームも豊富に取り揃えております。さらに、新規登録者限定の**[ウェルカムボーナス](LINK_NEEDS_REPAIR)** もご用意しております。初回入金額に応じて、様々なボーナスやフリースピンを獲得するチャンスがあります。<a href="https://www.casinotsu.com/bonuses" target="_blank" rel="noopener noreferrer"><strong>オンラインカジノによって提供される入金ボーナス</strong>は異なりますので、ぜひ下のリンクから詳細をご確認いただき、ご自身に合ったカジノを選んで、お得なボーナスを受け取ってください。</>,
    ],
    callout: {
      emoji: "📊",

...
export default function InformationHubSection() {
  return (
    <section>
      <div className="space-y-8">
        ...
        <div className="space-y-4">
          <h3 className="text-xl font-bold text-gray-900 flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-blue-600" /> 私たちの理念と方法論</h3>
          ...
          {/* Part 1 detail */}
          <div className="bg-white rounded-lg shadow-sm p-5 text-gray-700 leading-relaxed">
            <AuthorBadge name="Rina Okabe" role="編集方針・著者" avatarSrc="/rina/avatar.png" className="mb-3" />
            <p className="mb-3">
              私たちは国際的な読者に向けて、オンラインゲーミング産業を客観的に解説する教育的メディアです。法令遵守はユーザーの居住国・地域ごとに異なるため、各自で最新の法的要件を確認し、自己責任での利用をお願いします。
            </p>
            <p className="mb-3">
              すべてのコンテンツはデータドリブンで、私の一貫した基準と二重チェック体制に基づき作成しています。実際の登録～入金～プレイ～出金までの挙動を検証し、透明性の高い評価を行います。特に「セキュリティ・匿名性・秘匿性」の評価に重点を置き、<strong>暗号化（SSL/TLS）</strong>、<strong>プライバシーポリシー</strong>、<strong>支払い手段の秘匿性・速度</strong>、<strong>プライバシーツールのサポート</strong>を厳密に確認しています。
            </p>
            <p>
              また、悪質な運営や無許可事業者の情報はブラックリストとして定期的に更新し、ユーザー保護を最優先に発信しています。
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <h3 className="text-xl font-bold text-gray-900 flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-blue-600" /> 信頼＆安全のための必読ガイド</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <InfoCard
        ...
        <div className="space-y-4">
          <h3 className="text-xl font-bold text-gray-900 flex items-center gap-2"><FileText className="h-5 w-5 text-blue-600" /> アドバンスト知識＆業界分析</h3>
          ...
          {/* Part 3 detail */}
          <div className="bg-white rounded-lg shadow-sm p-5 text-gray-700 leading-relaxed">
            <AuthorBadge name="Hiroshi" role="データ分析・戦略" avatarSrc="/hiroshi/hiroshi.png" className="mb-3" />
            <p className="mb-3">
              私は初心者向けの基礎情報にとどまらず、<strong>暗号資産の安全な取り扱い</strong>や<strong>eウォレットの手数料・KYC比較</strong>、<strong>ゲーム数学に基づく最適戦略</strong>まで踏み込むことで、より高度なリテラシー醸成を目指しています。これにより、ユーザーは手段・コスト・リスクを定量的に比較し、効率的かつ安全な選択が可能になります。
            </p>
            <p>
              さらに、私は国際規制や技術革新のニュースを継続的に分析し、<strong>市場動向・規制の影響・新技術の実装</strong>をタイムリーにフォローしています。単なるレビューサイトを超え、意思決定に直結する「実務的な知見」をお届けします。
            </p>
          </div>
        </div>
      </div>
...
function InfoCard({
  ...
  return (
    <div className={`bg-white rounded-lg shadow-sm p-5 shadow-sm ${className}`}>
      ...
      {cta && (
        <div className="mt-3">
          <Link href={cta.href} className="text-blue-600 hover:text-blue-700 font-semibold text-sm">
            {cta.label} →
          </Link>
        </div>
      )}

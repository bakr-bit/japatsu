...
export type InstantWithdrawalCasinosPageData = {
  title: string;
  banner?: {
    title: string;
    subheading?: string;
  ...
  rankings?: Casino[]; // fastest payout casinos
  criteria?: string[]; // evaluation points for speed & reliability
  tips?: string[];
  faq?: Array<{ q: string; a: string }>;
};

export default function InstantWithdrawalCasinosTemplate({ data }: { data: InstantWithdrawalCasinosPageData }) {
  const d = data;
  return (
    <SectionScaffold title={d.title}>
      <div className="space-y-10">
        ...
        {d.criteria && d.criteria.length ? (
          <section>
            <SectionTitle title="評価基準（出金スピード）" as="h2" />
        ...
        {d.tips && d.tips.length ? (
          <section>
            <SectionTitle title="出金を早めるコツ" as="h2" />
            <ul className="space-y-2 text-gray-800">
              {d.tips.map((t, i) => (
                <li key={i} className="flex items-start gap-2">
                  <span className="mt-1 h-1.5 w-1.5 rounded-full bg-red-600" />

...
export type BaccaratPageData = {
  ...
  rules?: BacSection;
  strategy?: BacSection;
  variants?: Array<{ name: string; description?: string; link?: string }>;
  tables?: BacTable[];
  tips?: string[];
...
export default function BaccaratTemplate({ data }: { data: BaccaratPageData }) {
  const d = data;
  return (
    <SectionScaffold title={d.title}>
      <div className="space-y-10">
        ...
        {d.howto ? (
          <section>
            <SectionTitle title={d.howto.title || "基本の遊び方"} as="h2" />
        ...
        {(d.tips && d.tips.length) || d.prosCons ? (
          <section>
            <SectionTitle title="ヒント・メリデメ" as="h2" />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {d.tips && d.tips.length ? (
                <div className="rounded-lg border bg-white p-4">
                  <h3 className="font-semibold text-green-700 mb-2">ヒント</h3>
              ...
              {d.prosCons ? (
                <div className="rounded-lg border bg-white p-4">
                  <h3 className="font-semibold text-red-700 mb-2">メリット・デメリット</h3>

...
export type MostAwardsPageData = {
  title: string;
  banner?: {
    title: string;
    subheading?: string;
  ...
  rankings?: Casino[]; // top awarded casinos
  criteria?: string[];
  faq?: Array<{ q: string; a: string }>;
};

export default function MostAwardsTemplate({ data }: { data: MostAwardsPageData }) {
  const d = data;
  return (
    <SectionScaffold title={d.title}>
      <div className="space-y-10">
        ...
        {d.criteria && d.criteria.length ? (
          <section>
            <SectionTitle title="評価基準（受賞歴）" as="h2" />

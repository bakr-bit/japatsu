...
export type CasinoAppsPageData = {
  title: string;
  banner?: {
    title: string;
    subheading?: string;
  ...
  criteria?: string[]; // what makes a good casino app
  tips?: string[];
  faq?: Array<{ q: string; a: string }>;
  cta?: { text: string; href: string };
};

export default function CasinoAppsTemplate({ data }: { data: CasinoAppsPageData }) {
  const d = data;
  return (
    <SectionScaffold title={d.title}>
      <div className="space-y-10">
        ...
        {d.criteria && d.criteria.length ? (
          <section>
            <SectionTitle title="評価基準（カジノアプリ）" as="h2" />
        ...
        {d.tips && d.tips.length ? (
          <section>
            <SectionTitle title="利用のコツ" as="h2" />
            <ul className="space-y-2 text-gray-800">
              {d.tips.map((t, i) => (
                <li key={i} className="flex items-start gap-2">
                  <span className="mt-1 h-1.5 w-1.5 rounded-full bg-red-600" />

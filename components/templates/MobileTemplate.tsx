...
export type MobilePageData = {
  ...
  criteria?: string[]; // mobile UX criteria
  tips?: string[];
  faq?: Array<{ q: string; a: string }>;
};

export default function MobileTemplate({ data }: { data: MobilePageData }) {
  const d = data;
  return (
    <SectionScaffold title={d.title}>
      <div className="space-y-10">
        ...
        {d.criteria && d.criteria.length ? (
          <section>
            <SectionTitle title="評価基準（モバイル適性）" as="h2" />
        ...
        {d.tips && d.tips.length ? (
          <section>
            <SectionTitle title="活用のコツ" as="h2" />
            <ul className="space-y-2 text-gray-800">
              {d.tips.map((t, i) => (
                <li key={i} className="flex items-start gap-2">
                  <span className="mt-1 h-1.5 w-1.5 rounded-full bg-red-600" />

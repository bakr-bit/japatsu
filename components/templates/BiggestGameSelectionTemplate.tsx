...
export type BiggestGameSelectionPageData = {
  title: string;
  banner?: {
    title: string;
    subheading?: string;
  ...
  rankings?: Casino[]; // top casinos with huge game libraries
  criteria?: string[]; // how we evaluate game selection
  faq?: Array<{ q: string; a: string }>;
  cta?: { text: string; href: string };
};

export default function BiggestGameSelectionTemplate({ data }: { data: BiggestGameSelectionPageData }) {
  const d = data;
  return (
    <SectionScaffold title={d.title}>
      <div className="space-y-10">
        ...
        {d.criteria && d.criteria.length ? (
          <section>
            <SectionTitle title="評価基準（ゲームの充実度）" as="h2" />
